...
echo You're running 'manage_Vanguard.bat'
echo This script allows you to quickly enable/disable Vanguard individual services
...
sc qc "vgc" >nul
if ERRORLEVEL 1 (
    echo [WARNING]: vgc service is not present on this machine.
) else (
    ...
    for /f "tokens=3 delims=: " %%H in ('sc qc "vgc" ^| findstr /i "START_TYPE"') do (
        ...
        if /i "%%H" EQU "DEMAND_START" (
            choice /c ny /n /t 180 /d y /m "DO YOU WANT TO DISABLE vgc SERVIVCE STARTUP?[Y/n]    "
            if ERRORLEVEL 2 (
                echo.
                echo        Disabling vgc...
            ...
            choice /c ny /n /t 180 /d y /m "DO YOU WANT TO ENABLE vgc SERVIVCE STARTUP? [Y/n]    "
            if ERRORLEVEL 2 (
                echo.
                echo        Enabling vgc...
...
sc qc "vgk" >nul
if ERRORLEVEL 1 (
    echo [WARNING]: vgk service is not present on this machine.
) else (
    ...
    for /f "tokens=3 delims=: " %%H in ('sc qc "vgk" ^| findstr /i "START_TYPE"') do (
        ...
        if /i "%%H" EQU "SYSTEM_START" (
            choice /c ny /n /t 180 /d y /m "DO YOU WANT TO DISABLE vgk SERVIVCE STARTUP?[Y/n]    "
            if ERRORLEVEL 2 (
                echo.
                echo        Disabling vgk...
            ...
            choice /c ny /n /t 180 /d y /m "DO YOU WANT TO ENABLE vgk SERVIVCE STARTUP? [Y/n]    "
            if ERRORLEVEL 2 (
                echo.
                echo        Enabling vgk...
...
if ERRORLEVEL 2 (
    ...
    echo. && echo Please note that if you enabled any services you need to restart this machine for them to properly load. && echo.
)
